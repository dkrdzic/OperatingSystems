...
Thread *List::getThreadById(ID id)
{
	...
	while (cur != 0 && cur->pcb->myThread->id != id)
		cur = cur->next;
	if (cur == 0)
	{
		unlock;
	...
	{
		unlock;
		return (Thread *)cur->pcb->myThread;
	}
}
...
void List::checkTime()
{
	...
	while (cur)
		if (cur->pcb->blockingTimeSlice == 0)
		{
			before = cur;
			cur = cur->next;
		}
...
void List::readList()
{
	...
	while (cur != 0)
	{
		printf("Cekam semafor: %d, TS: %d, TC:%d, ID:%d \n", cur->pcb->idSem, cur->pcb->blockingTimeSlice,

			   cur->pcb->blockingTimeCount, cur->pcb->myThread->id);
		cur = cur->next;
	}
